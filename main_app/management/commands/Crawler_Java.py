...
class Command(BaseCommand):
    help = 'web crawler for Java'

    def handle(self, *args, **options):
        Error_var = False
        errors = []
        initial_version = False
        appname = "Oracle Java Runtime Enviroment"
        #logging
        ...
        #define our main url
        url = "https://www.java.com/en/download/win10.jsp"
        ...
        if Error_var != True:
            ...
            for ver in page_soup.find_all("h4", class_="sub"):
                version_text = str(ver.text)

            version_text = version_text.replace('Recommended Version', '')
            version_text = version_text.replace('Update', '.')
