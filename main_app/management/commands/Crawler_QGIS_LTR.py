...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        #define our main url
        url = "https://qgis.org/fi/site/forusers/download.html"
        downloadURL = url
        ...
        if Error_var != True:
            ...
            r = '\d\.\d{0,2}\.{0,1}\d{0,2} LTR'

            logging.info('sniffing through the html for regex pattern')
            repElemList = page_soup.find("span", {"class": "navbar-text pull-left"})

            version_text = re.findall(r, str(repElemList.text))

            version_text = [w.replace(" LTR", "") for w in version_text]

            versions_list = version_text
